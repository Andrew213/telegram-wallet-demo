  ...
  return {
    ...
    icon: Currency.getIcon(balance.alias)
      ? {
          ...
          testId: `payment-balance-${balance.id}-select-wallet-icon`,
        }
      : undefined,
    iconNode: !Currency.getIcon(balance.alias) ? (
      <div
        className="flex size-10 items-center justify-center rounded-4 bg-grey-400 text-white dark:bg-dark-text-tertiary dark:text-dark-bg"
        data-testid={`payment-balance-${balance.id}-select-wallet-icon`}>
