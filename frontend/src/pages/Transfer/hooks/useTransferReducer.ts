import {GetAccount, GetOutputPaymethods} from "@api/requests";
import {useReducer} from "react";

export default function useTransferReducer(
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    case "ChangeBalance": {
      if (state.balance.id === action.payload.id) {
        return state;
      }
    ...
    case "ChangePaymethod": {
      if (state.paymethod.id === action.payload.id) {
        return state;
      }
